...
try {
  ...
  const hasGetAll = server.includes("getAll") && server.includes("cookieStore.getAll");
  const hasSetAll = server.includes("setAll") && server.includes("cookieStore.set");
  const hasTryCatch =
    server.includes("try {") && server.includes("catch") && server.includes("setAll");
  const hasWarn = server.includes("console.warn") && server.includes("setAll");
...
try {
  for (const p of expectedCookieHandlers) {
    ...
    if (!usesCookies)
      fail(`${p} should use createSupabaseRouteHandler/createRouteContext (cookie-aware) for auth`);
