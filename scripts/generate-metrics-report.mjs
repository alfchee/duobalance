...
const report = [
  ...
  "Exposure = at least one article mount (source `guide-view`/`help-center`) before the user's first transaction; users with no transaction yet count as viewed if they ever mounted an article. Mounts after the first transaction do not count. Offline reads flushed on reconnect carry flush time (`flushStoredGuideOpens` does not send the stored event time), so an offline read followed by a transaction before reconnect can classify as unviewed — exposure may undercount offline readers. Medians use the same epoch-minute `percentile_cont(0.5)` as the distribution section and are noisy at small n.",
  "",
  section(
